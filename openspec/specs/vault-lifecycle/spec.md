...
- **THEN** the system creates `.storage/`, `.metadata/`, `.metadata/types/`, `.metadata/index.txt`, and `.metadata/vault.toml`
- **THEN** the vault.toml file SHALL contain a generated UUIDv7 vault identifier, creation timestamp, and schema version
...
- **WHEN** user runs `prism init` on a directory that already has `.metadata/vault.toml`
...
The system SHALL open a vault by reading its `.metadata/vault.toml` and verifying the internal structure.

#### Scenario: Open valid vault
- **WHEN** user runs any command in a valid vault directory
- **THEN** the system SHALL read `.metadata/vault.toml` and confirm the vault is intact
...
- **THEN** the system SHALL add the vault path and UUID to a vault registry file at `~/.config/prism/vaults.toml`
